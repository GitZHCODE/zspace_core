...
#include<zspace/zDisplay/model/zModel.h>
...
namespace zSpace
{
	...
	ZSPACE_INLINE void zModel::draw()
	{
		...
		{
			if (auto* mesh = dynamic_cast<zObjMesh*>(obj))
			{
				displayScene.draw(*mesh);
			}
			else if (auto* graph = dynamic_cast<zObjGraph*>(obj))
			{
				displayScene.draw(*graph);
			}
		}

		// buffer display

		if (showBufPoints) displayScene.drawBufferedPoints(showBufPointColors);

		if (showBufLines) displayScene.drawBufferedLines(showBufLinesColors);

		if (showBufTris) displayScene.drawBufferedTriangles(showBufTrisColors);

		if (showBufQuads) displayScene.drawBufferedQuads(showBufQuadsColors);
