...
#include <zspace/zInterface/objects/zObjectComputeMesh.h>
...
namespace  zSpace
{
	...
	*/
	...
	protected:
		...
		/*! \brief Pointer to a zObjectComputeMesh object*/
		zObjectComputeMesh* compMeshObj;
	...
	public:
		...
		*	\since version 0.0.2
		*/
		zFnComputeMesh(zObjectComputeMesh& _compMeshObj);
...
{
	extern "C" {
		void extCreateComputeMesh(double* _vertexPositions, int* _polyCounts, int* _polyConnects, int* _triCounts, int* _triConnects, int numVerts, int numFaces, zObjectComputeMesh* outComputeMesh);

		void extDeleteCreateComputeMesh(zObjectComputeMesh* computeMesh);
	}
...
#else
#include <src/zInterface/functionsets/zFnComputeMesh.cpp>
#endif
