...
namespace zSpace
{
	...
	ZSPACE_INLINE int zObjBuffer::appendVertexAttributes(zPoint*_positions, zVector* _normals, int size)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
				vertices[(i * vertexAttribStride) + 3] = 0;
				vertices[(i * vertexAttribStride) + 4] = 0;
				vertices[(i * vertexAttribStride) + 5] = 0;
			}
		}
	...
	ZSPACE_INLINE int zObjBuffer::appendVertexAttributes(float* _positions, float* _normals, int numPositions)
	{
		...
		GLfloat* vertices = new GLfloat[numPositions * vertexAttribStride];

		for (int i = 0; i < numPositions; i++)
		{
			vertices[(i * vertexAttribStride) + 0] = _positions[i * 3 + 0];
			vertices[(i * vertexAttribStride) + 1] = _positions[i * 3 + 1];
			vertices[(i * vertexAttribStride) + 2] = _positions[i * 3 + 2];


			if (_normals)
			{
				vertices[(i * vertexAttribStride) + 3] = _normals[i * 3 + 0];
				vertices[(i * vertexAttribStride) + 4] = _normals[i * 3 + 1];
				vertices[(i * vertexAttribStride) + 5] = _normals[i * 3 + 2];
			}
			else
			{
				vertices[(i * vertexAttribStride) + 3] = 0;
		...
		glBindBuffer(GL_ARRAY_BUFFER, VBO_vertices);
		glBufferSubData(GL_ARRAY_BUFFER, nVertices * vertexAttribStride * GLFloatSize, numPositions * vertexAttribStride * GLFloatSize, vertices);

		nVertices += numPositions;
	...
	ZSPACE_INLINE int zObjBuffer::appendVertexColors(float* _colors, int numPositions)
	{
		...
		for (int i = 0; i < numPositions; i++)
		{
			colors[(i * vertexColorStride) + 0] = _colors[i * 4 + 0];
			colors[(i * vertexColorStride) + 1] = _colors[i * 4 + 1];
			colors[(i * vertexColorStride) + 2] = _colors[i * 4 + 2];
			colors[(i * vertexColorStride) + 3] = _colors[i * 4 + 3];
		}


		glBindBuffer(GL_ARRAY_BUFFER, VBO_vertexColors);
		glBufferSubData(GL_ARRAY_BUFFER, nColors * vertexColorStride * GLFloatSize, numPositions * vertexColorStride * GLFloatSize, colors);

		nColors += numPositions;
	...
	ZSPACE_INLINE void zObjBuffer::updateVertexPositions(float* _positions, int numPositions, int startId)
	{
		...
		for (int i = 0; i < numPositions; i++)
		{
			positions[(i * vertexAttribStride) + 0] = _positions[i * 3 + 0];
			positions[(i * vertexAttribStride) + 1] = _positions[i * 3 + 1];
			positions[(i * vertexAttribStride) + 2] = _positions[i * 3 + 2];
		}

		glBindBuffer(GL_ARRAY_BUFFER, VBO_vertices);
		glBufferSubData(GL_ARRAY_BUFFER, startId * vertexAttribStride * GLFloatSize, numPositions * vertexAttribStride * GLFloatSize, positions);
	...
	ZSPACE_INLINE void zObjBuffer::updateVertexNormals(float* _normals, int numPositions, int& startId)
	{
		...
		for (int i = 0; i < numPositions; i++)
		{
			normals[(i * vertexAttribStride) + 3] = _normals[i * 3 + 0];
			normals[(i * vertexAttribStride) + 4] = _normals[i * 3 + 1];
			normals[(i * vertexAttribStride) + 5] = _normals[i * 3 + 2];
		}

		glBindBuffer(GL_ARRAY_BUFFER, VBO_vertices);
		glBufferSubData(GL_ARRAY_BUFFER, startId * vertexAttribStride * GLFloatSize, numPositions * vertexAttribStride * GLFloatSize, normals);
	...
	ZSPACE_INLINE void zObjBuffer::updateVertexColors(float* _colors, int numPositions, int& startId)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, VBO_vertexColors);
		glBufferSubData(GL_ARRAY_BUFFER, startId * vertexColorStride * GLFloatSize, numPositions * vertexColorStride * GLFloatSize, colors);
