...
#include <zspace/zInterface/functionsets/zFnComputeMesh.h>
...
namespace  zSpace
{
	...
	ZSPACE_INLINE zFnComputeMesh::zFnComputeMesh(zObjectComputeMesh& _computeMeshObj)
	{
		fnType = zFnType::zComputeMeshFn;
	...
	ZSPACE_INLINE bool zFnComputeMesh::create(double* _vertexPositions, int* _polyCounts, int* _polyConnects, int* _triCounts, int* _triConnects, int numVerts, int numFaces)
	{
		...
		compMeshObj = new zObjectComputeMesh();
		//set all counts
	...
	ZSPACE_INLINE void zFnComputeMesh::setParticles()//, int numVerts,  zPoint* vertexPositions, zObjectParticle* o_Particles, zFnParticle* fn_Particles, zPoint* vertexPositions)
	{

		compMeshObj->o_Particles = new zObjectParticle[compMeshObj->nV];
